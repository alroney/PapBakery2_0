...
//#region - USER RELATED API ENDPOINTS
    ...
    const fetchUser = async (req,res,next) => {
        const token = req.header('auth-token'); //req.header(keyname);
        if(!token) {
            res.status(401).send({errors: "Please authenticate using valid token."}); //Set status code to 401 and send an error object where key is errors.
        }
        else {
            try {
                const data = jwt.verify(token, 'secret_ecom'); //The token will be decoded use the salt (secret_ecom) which will be saved into the variable data.
                req.user = data.user;
                next();
            }
            catch (error) {
